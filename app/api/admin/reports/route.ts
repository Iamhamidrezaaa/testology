...
export async function GET(req: NextRequest) {
  try {
    // بررسی session (NextAuth)
    let session = await getServerSession(authOptions)
    
    // اگر session نداریم، از header یا query params چک کن (برای localStorage auth)
    if (!session || !session.user) {
      // برای حالتی که از localStorage استفاده می‌شود
      // در این حالت، فقط بررسی می‌کنیم که درخواست از ادمین است
      // این یک راه حل موقت است تا زمانی که NextAuth کامل پیاده‌سازی شود
